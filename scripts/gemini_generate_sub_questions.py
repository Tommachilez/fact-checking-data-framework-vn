...
# Format prompt for Gemini API call
def construct_prompt(showcase_examples, context):
    return showcase_examples.format(context)


# Generate sub-questions with Gemini API and apply filtering
def generate_questions_by_gemini(context, questions_by_gemini):
    ...
    for q in res.splitlines():
        ...
        if len(q) != 0:
            ...
            if not q.isspace() and not any(x in q.lower() for x in WH_MATCHES):
                for q_alreay_in in questions_by_gemini:
            ...
            if is_added:
                questions_by_gemini.append(q)
    return questions_by_gemini
...
def main(args):
    ...
    for i in tqdm(range(start, end)):
        ...
        try:
            ...
            gemini_called = 0
            questions_by_gemini = []
            while (len(questions_by_gemini) < MAX_NUM_QUESTIONS
                   and gemini_called < MAX_GPT_CALLS):
                questions_by_gemini = generate_questions_by_gemini(
                    context, questions_by_gemini
                )
                gemini_called += 1

            df.at[i, 'qg-output'] = questions_by_gemini
